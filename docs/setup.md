...
Installs `omp`, `gh` shim, credential helper, fence in `~/.local/libexec/devbox-agent`; `devbox-gh-token`
in `~/.local/bin`; `~/.local/bin/omp` symlinked to the launcher; `~/.config/devbox/agent*.gitconfig`;
