...
# work as yourself runs over the laptop's forwarded 1Password agent, and the
...
# names as the signing key. Agent sessions never see the forwarded agent: their
# git goes over HTTPS with per-operation tokens (§12).
mkdir -p "${SSH_DIR}"
chmod 700 "${SSH_DIR}"
for identity in personal work; do
  key="${SSH_DIR}/id_${identity}"
  var="GIT_$(printf '%s' "${identity}" | tr '[:lower:]' '[:upper:]')_PUBKEY"
  ...
  if [[ -n "${!var:-}" ]]; then
    ...
    register_action "Set ${var} in .env to the laptop's ${identity} public key (cat ~/.ssh/<key>.pub), then ./bin/devbox up - until then ${identity} manual git work over a forwarded agent cannot pick its key"
  fi
done
...
# are unsigned by design (§12).
{
  for identity in personal work; do
    ...
    [[ -f "${SSH_DIR}/id_${identity}.pub" ]] && printf '%s %s\n' "${!var:-}" "$(cut -d' ' -f1,2 "${SSH_DIR}/id_${identity}.pub")"
  done
