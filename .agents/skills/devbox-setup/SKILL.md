...
The devbox generates no keys - it installs only the laptop's public keys from `.env` (`GIT_*_PUBKEY` auth,
`GIT_*_SIGNINGKEY` signing). Nothing to register on GitHub - already your normal, registered laptop keys.

1. Set `GIT_PERSONAL_PUBKEY`/`GIT_WORK_PUBKEY` (auth), `GIT_PERSONAL_SIGNINGKEY`/
   `GIT_WORK_SIGNINGKEY` (signing) to the public keys `git config user.signingkey` prints on the laptop,
   then `./bin/devbox up` - until then, manual git over a forwarded agent (`ssh -A devbox`) can't pick a key,
   commits sign with nothing. A prior bootstrap's private key is deleted this run, fingerprint printed to
   revoke.
2. Put **two fine-grained** GitHub tokens in `~/.config/devbox/secrets.env`: `GH_TOKEN_PERSONAL`,
   `GH_TOKEN_WORK` - `contents: write` on repos agents push to without the GitHub App, plus
   `actions`/`checks` read, `issues`/`pull-requests` write only if agents should post. Working directory
   picks which (`~/projects/work/**` = work, like git identities); `devbox-gh-token --account`
   reports it. Skip `gh auth login`: its web flow can't scope below `repo` + `read:org` + `gist` -
   non-expiring, account-wide, plaintext (no keyring here). A box-wide `GH_TOKEN` works but overrides both,
   killing the per-directory choice.
...
4. Place work-app GitHub App credentials in `~/.config/work/work-app/` (`app-id`, `app.pem`,
   mode 600) if needed. Bootstrap creates the directory, never fetches secrets; once present, the
   credential helper mints a repo-scoped installation token per agent git op, ahead of the PAT.
...
`Hi rozsival-work!`. Agent sessions (`omp` launcher) skip this, pushing HTTPS with a token minted per
operation. Full mechanism: `docs/git.md`.
